...
export default class AccountInfoDemo extends LightningElement {
    ...
    startAutoRefresh() {
        this.intervalId = setInterval(() => {
            if (this.wiredOppResponse) {
                console.log('Refresh');
                refreshApex(this.wiredOppResponse);
            }
        }, 10000);
    ...
    handleSave(event) {
        ...
        updateOpportunities({ oppList: updatedFields })
            .then(() => {
                ...
                // Clear the yellow highlights and save/cancel footer
                this.draftValues = [];

                // Re-run the wire logic to update the DisplayName prefixes
    ...
    handleClick(event) {
        ...
        this.accounts = this.accounts.map(acc => {
            let label = 'Show Opportunity';
            let variant = 'brand';
            if (acc.Id === this.selectedAccountId && this.showOpportunities) {
                label = 'Hide Opportunity';
                variant = 'destructive';
            }
            return { ...acc, btnLabel: label, btnVariant: variant };
        });
